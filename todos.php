...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['action'] === 'add') {
        ...
        if ($status == 'completed' && $old['status'] != 'completed' && $notifications) {
            $email = $pdo->prepare("SELECT email FROM users WHERE id = ?")->execute([$user_id]); $email = $pdo->fetchColumn();
...
$params = [$user_id];
if ($_GET['search'] ?? '') { $s = "%{$_GET['search']}%"; $where .= " AND (t.title LIKE ? OR t.description LIKE ?)"; $params[] = $s; $params[] = $s; }
...
$stmt = $pdo->prepare("SELECT t.*, c.name as cat_name FROM todos t LEFT JOIN categories c ON t.category_id = c.id $where ORDER BY t.created_at DESC LIMIT $limit OFFSET $offset");
$limit = (int)$limit; 
$offset = (int)$offset;
$stmt->execute($params);
$todos = $stmt->fetchAll();

$stmt2 = $pdo->prepare("SELECT * FROM categories WHERE user_id = ?");
...
<div class="container mt-5">
    ...
    <form method="GET" class="row g-2 mb-3">
        <div class="col-md-4"><input type="text" name="search" class="form-control" placeholder="Search..." value="<?php echo $_GET['search'] ?? ''; ?>"></div>
    ...
    <table class="table table-striped"> <thead><tr><th>Title</th><th>Status</th><th>Due</th><th>File</th><th>Actions</th></tr></thead>
        <tbody>
            <?php foreach ($todos as $t): ?>
            <tr>
                <td><?php echo htmlspecialchars($t['title']); ?></td>
                <td><span class="badge bg-<?php echo $t['status']=='completed'?'success':($t['status']=='in_progress'?'info':'warning'); ?>">
                    <?php echo $t['status']; ?>
                </span></td>
    ...
    <nav><ul class="pagination">
        ...
        <li class="page-item <?php echo $page==$i?'active':''; ?>"><a class="page-link" href="?page=<?php echo $i; ?>&<?php echo http_build_query(['search'=>$_GET['search']??'', 'status'=>$_GET['status']??'', 'category'=>$_GET['category']??'']); ?>"><?php echo $i; ?></a></li>
        <?php endfor; ?>
    </ul></nav>
</div>
...
<div class="modal fade" id="addModal"><div class="modal-dialog modal-lg"><div class="modal-content">
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-footer"><button type="submit" class="btn btn-success">Save</button><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button></div>
    </form>
</div></div></div>
